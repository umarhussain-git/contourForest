...
#'
#' This function generates a contour-enhanced forest plot for binary outcomes, such as Odds Ratios (OR) or Risk Ratios (RR), using data from intervention and control groups. The plot highlights statistical significance contours and optionally prediction intervals.
#'
#' @param dat A data frame containing the study-level data for meta-analysis.
#' @param measure Character string specifying the effect size measure. Options: "OR" (Odds Ratio) or "RR" (Risk Ratio). Default is "OR".
#' @param method Method for pooling effect sizes. Typically "REML", "DL", or "ML". Default is "REML".
#' @param xlab Label for the x-axis. Default is an empty string.
#' @param title Plot title. Default is NULL (no title).
#' @param model Character string specifying model type: "Random-effects" or "Fixed-effects". Default is "Random-effects".
#' @param estimator Estimator for between-study variance. Default is "REML".
#' @param nc_col Column name in `dat` for number of participants in the control group.
#' @param ne_col Column name in `dat` for number of participants in the experimental/treatment group.
#' @param event_c_col Column name in `dat` for events in the control group.
#' @param event_t_col Column name in `dat` for events in the treatment group.
#' @param diamond.col Color for the summary effect diamond. Default is "blue".
#' @param study.col Color for individual study points. Default is "black".
#' @param CI.col Color for confidence interval lines. Default is "black".
#' @param Pred.Inter.col Color for prediction interval lines. Default is "red".
#' @param square.size Numeric value specifying the size of study effect squares. Default is 3.
#' @param contour_fill Fill colors for significance contours. Can be a vector of colors. Default is c("#E0E0E0", "#B0B0B0").
#' @param text_size Numeric value for text size in the plot. Default is 3.
#' @param xlim Numeric vector of length 2 specifying x-axis limits. Default is NULL (automatic).
#' @param pred Logical indicating whether to show prediction interval. Default is TRUE.
#' @param xpos Numeric value for horizontal position of text annotations. Default is 1.
#' @param study_x Numeric value specifying x-position for study labels. Default is NULL (automatic).
#' @param hetero_x Numeric value specifying x-position for heterogeneity stats. Default is NULL (automatic).
#'
#' @import ggplot2
